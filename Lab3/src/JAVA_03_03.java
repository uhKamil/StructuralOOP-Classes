...
int DigitAtPosition(int n, int pos) {
    int position = CountDigits(n);
    int digit = 0;
    while (n > 0) {
        digit = n % 10;
        if (pos == position) {
...
void outputDigits(int n) {
    for (int i = CountDigits(n); i >= 1; i--) {
        print(DigitAtPosition(n, i) + "\n");
    }
}
...
void main() {
    ...
    int number;
    print("Enter the number and I'll output all its digits: ");
