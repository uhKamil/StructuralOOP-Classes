...
TStudentContainer GroupStudentsBySex(TUniversity univ) {
...
void DisplayBySex(TStudentContainer sbs) {
...
private static int compareStudents(TStudent s1, TStudent s2, TSortCriteria criterion) {
    switch (criterion) {
        case SURNAME:
            return s1.personal_data.surname.compareTo(s2.personal_data.surname);
        case WEIGHT:
            return s1.personal_data.weight - s2.personal_data.weight;
        case ADDRESS:
            return s1.personal_data.home_address.street.compareTo(s2.personal_data.home_address.street);
        case CITY:
            return s1.personal_data.home_address.city.compareTo(s2.personal_data.home_address.city);
        default:
            return 0;
    }
}
...
void main() {
    ...
    TStudentContainer GroupedStudents = GroupStudentsBySex(univ1);
    DisplayBySex(GroupedStudents);

//    println("\n4. Sorting the list of students by a criterion");
//    TStudent[] SortedStudents = sortStudents(GroupedStudents, TSortCriteria.CITY);
