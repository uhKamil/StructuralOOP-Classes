...
public static class TrainSimulation {
    ...
    static TSimulation initSimulation() {
        ...
        sim.track = createTrack(50000, 80);
        sim.time = 0;
        ...
        sim.trains = new Train[]{
                createTrain(1, 500, 0.5, 30, -1, "1"),
                createTrain(2, 2500, 0.2, 20, 1, "2"),
                createTrain(3, 50000, 0.7, 40, -1, "3"),
                createTrain(4, 10000, 0.4, 25, 1, "4"),
                createTrain(5, 40000, 0.6, 35, -1, "5"),
                createTrain(6, 25000, 0.9, 50, 1, "6")
        };
        return sim;
    }
    
    static Train createTrain(int id, double startX, double acc, double maxV, int dir, String sym) {
        Train t = new Train();
        t.id = id;
        t.x = startX;
        t.prev_x = startX;
        ...
        t.max_v = maxV;
        t.direction = dir;
        t.about_to_crash = false;
        t.crashed = false;
        t.symbol = sym;
        return t;
    }
    
    static Track createTrack(double lengthX, int widthW) {
        Track tr = new Track();
        tr.lengthX = lengthX;
        tr.widthW = widthW;
        return tr;
