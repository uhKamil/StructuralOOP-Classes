...
void main() {
    ...
    while (true) {
        ...
        if (game_dir == 0) game_pos++;
        else game_pos--;
        if (press_dir == 0) press_pos++;
        else press_pos--;

        // Collision check
        int normGame = (game_pos % path1.count + path1.count) % path1.count;   // [0, count-1]
        int normPress = (press_pos % path1.count + path1.count) % path1.count; // [0, count-1]

        int rawDist = Math.abs(normGame - normPress);
        int ringDist = Math.min(rawDist, path1.count - rawDist);

        if (ringDist <= 2) {
            game_dir = 1 - game_dir;
            press_dir = 1 - press_dir;
            
            if (game_dir == 0) game_pos += 1;
            else game_pos -= 1;
            if (press_dir == 0) press_pos += 1;
            else press_pos -= 1;
        }
