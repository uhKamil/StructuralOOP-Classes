...
// /* This is a version before the separation of classes' methods */
package Ex6;
...
public class JAVA_06_06_backup {
    static class Square {
        ...
        public Square(int boardWidth, int boardHeight, double gravity) {
        ...
        public void randomizeVelocity(int boardHeight) {
            this.dx = 0;
            this.dy = -(gravity * (boardHeight - this.y) / 5); // assigning potential energy
            // assigning kinetic energy
            while (this.dx == 0) this.dx = (int) (Math.random() * 3) - 1;
            this.dy += (double) (int) ((Math.random() * 3) - 2) / 2;
        }
...
//            else if (this.y > targetY) this.move_dir = moveDirection.UP;            
    ...
    static class SimulationModel {
        ...
        public SimulationModel(int count, int width, int height, Player[] players, double gravity) {
            this.width = width;
            this.height = height;
            this.count = count;
            ...
            for (int i = 0; i < count; i++) {
                this.squares[i] = new Square(width, height, gravity);
            }
        ...
        public boolean updateSquares() {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (s1.active) {
                    ...
                    boolean hitWallLeftX = nextX <= 1;
                    boolean hitWallRightX = (nextX + s1.getSize() >= width);
                    boolean hitTopWallY = nextY <= 2;
                    boolean hitBottomWallY = (nextY + s1.getSize() >= height + 1);

                    if (hitWallLeftX || hitWallRightX || hitTopWallY || hitBottomWallY) {
                        collisionEvent = true;
                        collided = true;
                        if (hitWallLeftX) {
                            s1.bounceX();
                            s1.setX(2);
                        } else if (hitWallRightX) {
                            s1.bounceX();
                            s1.setX(width - s1.getSize());
                        }
                        if (hitBottomWallY) {
                            s1.bounceY();
                            s1.setY(height - s1.getSize());
                        } else if (hitTopWallY) {
                            s1.bounceY();
                            s1.setY(2);
                        }
...
//        plr.setMoveDir(null);
    ...
    static class SimulationView {
        ...
        public void clearSquares(Square[] squares, int boardWidth, int boardHeight) {
            setfgcolor(0);
            for (Square s : squares) {
                if (!s.isCleaned()) {
                    discreteCoords c = getDiscreteCoords(boardWidth, boardHeight, s);
                    framexyc(c.sX(), c.sY(), (int) (c.sX() + s.getSize() - 1), (int) (c.sY() + s.getSize() - 1), ' ');
                    if (!s.active) {
                        s.setCleaned(true);
                    }
                }
        ...
        private discreteCoords getDiscreteCoords(int boardWidth, int boardHeight, Square s) {
            int sX, sY;
            if (s.getX() > boardWidth - 1) sX = (int) (boardWidth - s.getSize());
            else sX = (int) s.getX();
            if (s.getY() > boardHeight - 1) sY = (int) (boardHeight - s.getSize());
            else sY = (int) s.getY();
            return new discreteCoords(sX, sY);
        }

        private record discreteCoords(int sX, int sY) {
        }


        public void drawSquares(Square[] squares, int boardWidth, int boardHeight) {
            for (Square s : squares) {
                if (s.active) {
        ...
        public void renderClear(SimulationModel model) {
            clearSquares(model.getSquares(), model.width, model.height);
            for (Player plr : model.players) clearPlayer(plr);
        }

        public void renderDraw(SimulationModel model) {
            drawSquares(model.getSquares(), model.width, model.height);
    ...
    static class SimulationController {
        ...
        public static final int roundLength = 180; // round duration in seconds
        ...
        public SimulationController(int squareCount, Player[] players) {
            this.model = new SimulationModel(squareCount, 120, 30, players, 0.15);
